...
//! - Dictionary columns accept the same value variants as their value type
//!   (for example, `Str` for `Dictionary(_, Utf8)` or `Bin` for
//!   `Dictionary(_, FixedSizeBinary(_))`). Keys are managed by the builder.
//! - `FixedSizeBinary(w)` requires `Bin` values with exact length `w`.
//! - `List` is used for both `List` and `LargeList` logical types; the builder
//!   selects the correct offset width. `FixedSizeList` must match the declared
//!   list length.
...
pub enum DynCell {
    ...
    Str(String),
    /// Arbitrary bytes for `DataType::Binary`, `DataType::LargeBinary`, or `DataType::FixedSizeBinary(w)`
    /// (length must equal `w`) and their dictionary forms.
    Bin(Vec<u8>),
    // Nested
